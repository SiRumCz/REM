...
def draw_dependabot_issue_rem_graph(r_G: nx.DiGraph, d_G: nx.DiGraph, pos: dict, metric: str='final', title: str='', html_out: str=None, img_out: str=None):
    ...
    if len(Xv_d) > 1:
        data+=[go.Scatter(x=Xv_d[:-1],
               ...
               mode='markers',
               legendgroup="dev",
               name='dependencies required during build (blue ring/bigger circle means direct dependencies, red ring means problematic)',
    ...
    if len(Xv_r) > 1:
        data += [go.Scatter(x=Xv_r[:-1],
               ...
               mode='markers',
               legendgroup="rt",        
               name='dependencies required during use (blue ring/bigger circle means direct dependencies, red ring means problematic)',       
